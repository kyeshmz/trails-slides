...
12. ためになる×実用性チャート(`05-goal-chart.md`, chart)— 実用性は相対的
13. 3 文字キャリア年表(`03-education.md`, timeline)— ICU → CSL → MIT → ???(KFC/IBM)
14〜18. お金(`10-money-cover.md` 扉 + `11`〜`14`)
19〜22. 環境 / 旅 / 仕事 / ストーリーテリング(各 `*-cover.md`, cover)
...
| `chart`     | 2 軸チャート(散布図) | `x:` `y:` 軸ラベル、本文 `- ラベル \| x \| y`(0〜100)。`*` 始まりはアクセント色、`link: A -- B` で 2 点を破線でつなぐ |
...
`layout.js` の `applyMasterTemplate()` が `renderSlide()` 経由で全スライドに
フッターを付ける:左下に「Kye Shimizu — スライドタイトル」(小さい文字)、
右下にページ番号、その上に区切りの細線。スライド内(1280x720)に描画される
ためウィンドウと一緒に拡大縮小される。名前を変えるときは `layout.js` の
`AUTHOR` 定数を編集する。各レイアウトの下端パディングはフッター分
(約 60px)を空けておくこと。
