...
- `src/layout.js` — レイアウトエンジン(`content` / `goal` / `timeline` / `cover` / `galleryItem` / `board`)
- `src/main.js` — 読み込み・ナビゲーション・ギャラリー展開・もくじ挿入・ホームボタン・アニメーション

すべての要素は表示時に順番にアニメーションします(`data-animate` の
ステガー入場 + 背景写真のスローズーム)。
- `src/theme.js`, `src/dom.js` — デザイントークンと DOM ヘルパー

並びは `自己紹介` → `もくじ(ボード)` → `ゴール` → `3 文字キャリア年表` →
`作品ギャラリー(8 枚)` → `お金(扉 + 4 枚)` → `環境` → `旅` → `仕事` →
`ストーリーテリング`。
